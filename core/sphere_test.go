...
func TestSphereCollision(t *testing.T) {
	...
	detectedCollision := IsColliding(s1, s1)
	if detectedCollision != true {
		t.Errorf("IsColliding() != expected, returned %+v instead of true", detectedCollision)
	}
}
...
func TestSphereCollision_Seperated(t *testing.T) {
	...
	if detectedCollision != expected {
		t.Errorf("IsColliding() != expected, returned %+v instead of %+v", detectedCollision, expected)
	}
}
